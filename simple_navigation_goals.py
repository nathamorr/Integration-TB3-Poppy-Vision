...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            if(rospy.get_param("/targetLabel") == 1):

                rospy.set_param("/targetLabel", -1)
            
                # Navigation vers la position 1
                print("NAVIGATION HOME->1")
                if(not nav_goals.go_to(2.95, -6.5, 2)):
                    break

		# Pause pour le déchargement du robot
                rospy.sleep(3)

                # Navigation vers la position de depart
                print("NAVIGATION 1->HOME")
                if(not nav_goals.go_to(1.25,-2.9,-7.6)):
                    break

		# Ajustement de la position de départ
                if(not nav_goals.go_to(1.25,-2.83,-7.6)):
                    break

                print("navigation | robotReady -> True")
                rospy.set_param("robotReady", True)
                
            if(rospy.get_param("/targetLabel") == 2):

                rospy.set_param("/targetLabel", -1)
                
                # Navigation vers la position 2
                print("NAVIGATION HOME->2")
                if(not nav_goals.go_to(1.95,-7,2)):
                    break
		...
		# Ajustement de la position de départ
                ...
                print("s_navigation | waiting for targetLabel")
